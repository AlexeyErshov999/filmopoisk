...
function getActor(actorId) {
  if (!actorId)
    return Promise.reject(new Error("actor id is required to fetch an actor"));
  return axios
    ...
    .catch((error) => {
      ...
      return Promise.reject(new Error("failed to get actor for id:" + actorId));
    });
}
...
const isEmptyQuery = (query) =>
  !query ||
  (!query.has("title") && !query.has("release_year") && !query.has("genre"));
...
export const movieIdMap = new Map();
...
router.post("/api/v1/rateMovie", {}, (request, response) => {
  ...
  if (!token || !token.startsWith("Bearer ") || !isAuth(token.split(" ")[1])) {
    response.statusCode = STATUS.UNAUTHORIZED;
    return response.end(
    ...
    request.on("end", () => {
      ...
      if (isNaN(parsedUserRate) || parsedUserRate < 1 || parsedUserRate > 5) {
        response.statusCode = STATUS.ERROR_BAD_REQUEST;
        return response.end(
          JSON.stringify({
